...
async function setupUsuarios() {
  ...
  try {
    ...
    const [tables] = await connection.query(
      "SHOW TABLES FROM concejo_guachucal LIKE 'modulos'"
    );
    ...
    const [tables2] = await connection.query(
      "SHOW TABLES FROM concejo_guachucal LIKE 'usuarios_permisos'"
    );
    ...
    const [modulos] = await connection.query(
      'SELECT COUNT(*) as count FROM concejo_guachucal.modulos'
    );
    ...
    console.error('   3. Verifica que la base de datos "concejo_guachucal" exista');
