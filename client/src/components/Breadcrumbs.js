...
const Breadcrumbs = () => {
  ...
  if (pathnames.length === 0 || (pathnames.length === 1 && pathnames[0] === 'concejoguachucal')) {
    return null;
  }
  ...
  const buildPath = (index) => {
    ...
    return process.env.NODE_ENV === 'production' ? `/concejoguachucal${path}` : path;
  };
    ...
    <nav className="breadcrumbs" aria-label="Breadcrumb">
      <ol className="breadcrumbs-list">
        <li className="breadcrumb-item">
          <Link to={process.env.NODE_ENV === 'production' ? '/concejoguachucal' : '/'} className="breadcrumb-link">
